 ...
 *
 * Usage:
 *   npx ts-node ... apply-batch-migration.ts --file=../../scripts/catalog/batches/batch1-pupitr-stul.json
...
async function main() {
  ...
  for (const item of items) {
    ...
    const productData: { name?: string; categoryId?: string } = {};
    ...
    if (item.proposed.modificationName && existing.modifications.length) {
    ...
    if (item.proposed.elementName && existing.elements.length) {
